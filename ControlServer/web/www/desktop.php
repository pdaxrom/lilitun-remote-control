...
if (@$_REQUEST['userId_submitted']) {
    $_SESSION['userId'] = trim(addslashes($_REQUEST['newUserId']));

    if (preg_match('/^[0-9A-F]{8}-[0-9A-F]{4}-4[0-9A-F]{3}-[89AB][0-9A-F]{3}-[0-9A-F]{12}$/i', $_SESSION['userId']) !== 1) {
	if ($_SESSION['userId'] !== "") {
	    $_SESSION['errUserId'] = " (Invalid user ID! Used autogenerated)";
	}
...
<form action="desktop.php">
  ...
  <input type="text" id="userId" name="userId" value="<?php echo $userId; ?>" disabled><br>
  <label for="newUserId">Existing User Id</label>
  <input type="text" id="newUserId" name="newUserId" value=""><br>
