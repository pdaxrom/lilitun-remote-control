...
void Fl_SVG_Image::rasterize_(int W, int H) {
  ...
  d(4);
  if (to_desaturate_) Fl_RGB_Image::desaturate(to_desaturate_f);
...
Fl_Image *Fl_SVG_Image::copy(int W, int H) {
  ...
  svg2->to_desaturate_f = to_desaturate_f;
...
void Fl_SVG_Image::desaturate(float f) {
  to_desaturate_ = true;
  to_desaturate_f = f;
  Fl_RGB_Image::desaturate(f);
}
