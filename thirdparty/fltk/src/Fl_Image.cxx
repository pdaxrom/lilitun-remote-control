...
//void Fl_Image::desaturate(float f) {
//}
...
void Fl_RGB_Image::desaturate(float f) {
  // Don't desaturate an empty image...
  if (!w() || !h() || !d() || !array) return;
  ...
  for (new_ptr = new_array, old_ptr = array, y = 0; y < h(); y ++, old_ptr += line_i) {
    for (x = 0; x < w(); x ++, old_ptr += d()) {
      ...
      uchar L = (uchar) ((float)R * 0.3 + (float)G * 0.6 + (float)B * 0.1);

      *new_ptr++ = (uchar)(R + f * (L - R));
      *new_ptr++ = (uchar)(G + f * (L - G));
      *new_ptr++ = (uchar)(B + f * (L - B));
